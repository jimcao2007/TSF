...
class Dispatch
{
    ...
    protected static $http_req;
    protected static $http_resp;
    ...
    public static function filter($req_data)
    {
        ...
        foreach($req_data as $k=>$v)
        {
            ...
            {
                $req_data[$k] = Util\LibString::filterContent($v);
            }
        }
        return $req_data;
    }
     ...
     * @return array
     */
    public static function route($con,$act,$req_data,$post_body=null,$get=null,$http_obj)
    {
        $con_class_name = 'Controller\\'.$con;

        if(!class_exists($con_class_name))
        {
            return Dispatch::format(self::ERR_PARAMS,'con name err');
        }
        ...
        {
            return Dispatch::format(self::ERR_PARAMS,'act name err :'.$act);
        }

        ob_start();

        //先调用init
        $ret = $controller_class->init();
        if(!empty($ret) && $ret['status'] == self::SUCCESS)
        {
            //执行act
            $ret = $controller_class->$act();
            if(empty($ret))
            {
                $ret = Dispatch::format();
            }
        ...
        {
            $ret = Dispatch::formatError(\Dispatch::ERR_FORBIDDEN);
        }
        $resp_content = ob_get_contents();
        ob_end_clean();

        if($controller_class->display == false)
        {
            self::jsonOut($ret,$http_obj);
        }
        else
        {
            $http_obj->send($resp_content);
        }
        \Logger::flush();
    }
     ...
     * @return bool
     */
    public static function jsonOut($data,$http_obj,$ob_content='')
    {
        $http_obj->setHeader('Content-Type','application/json; charset=utf-8');
        $http_obj->send($ob_content.json_encode($data,JSON_PRETTY_PRINT |
