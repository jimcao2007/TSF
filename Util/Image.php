...
class Image
{
    ...
    public function open(){
        ...
        //\Logger::log('flow',"img file name ".$imgname);

        //检测图像文件
        if(!is_file($imgname))
        {
            \Logger::errMsg('不存在的图像文件');
            return false;
        }
        ...
        //\Logger::log('flow',"info ".var_export($this->info,true));

        if($this->info['type'] == 'gif')
        {
            \Logger::errMsg('图片类型非法');
            return false;
        }
